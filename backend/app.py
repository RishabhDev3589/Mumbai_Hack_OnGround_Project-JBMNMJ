...
# --- CSV export endpoint ---
from flask import send_file  # add at top imports if not present
import io, csv
...
def export_csv():
    """
    Export current user's transactions, bills, saving goals and contributions as a single CSV file.
    """
    ...
    writer.writerow(['id', 'type', 'amount', 'description', 'created_at'])
    transactions = Transaction.query.filter_by(user_id=current_user.id).order_by(Transaction.id).all()
    for t in transactions:
        created = t.created_at.isoformat() if getattr(t, 'created_at', None) else ''
        # adapt attribute names if your model uses different names
        writer.writerow([t.id, getattr(t, 'type', ''), getattr(t, 'amount', ''), getattr(t, 'description', ''), created])
    writer.writerow([])

    # --- Bills ---
    writer.writerow(['Bills'])
    writer.writerow(['id', 'name', 'amount', 'due_date', 'paid', 'created_at'])
    bills = Bill.query.filter_by(user_id=current_user.id).order_by(Bill.id).all()
    for b in bills:
        due = b.due_date.isoformat() if getattr(b, 'due_date', None) else ''
        created = b.created_at.isoformat() if getattr(b, 'created_at', None) else ''
        writer.writerow([b.id, getattr(b, 'name', ''), getattr(b, 'amount', ''), due, getattr(b, 'paid', False), created])
    writer.writerow([])

    # --- Saving Goals + Contributions ---
    writer.writerow(['SavingGoals'])
    writer.writerow(['goal_id', 'name', 'target_amount', 'committed_date', 'target_months', 'created_at'])
    goals = SavingGoal.query.filter_by(user_id=current_user.id).order_by(SavingGoal.id).all()
    for g in goals:
        committed = g.committed_date.isoformat() if getattr(g, 'committed_date', None) else ''
        created = g.created_at.isoformat() if getattr(g, 'created_at', None) else ''
        writer.writerow([g.id, getattr(g, 'name', ''), getattr(g, 'target_amount', ''), committed, getattr(g, 'target_months', ''), created])

        # contributions per goal
        writer.writerow([f'Contributions for goal_id={g.id}'])
        writer.writerow(['contrib_id', 'month_index', 'contributed', 'contributed_amount', 'recorded_at'])
        contributions = SavingContribution.query.filter_by(goal_id=g.id).order_by(SavingContribution.id).all()
        for c in contributions:
            recorded = c.recorded_at.isoformat() if getattr(c, 'recorded_at', None) else ''
            writer.writerow([c.id, getattr(c, 'month_index', ''), getattr(c, 'contributed', ''), getattr(c, 'contributed_amount', ''), recorded])
        writer.writerow([])

    # prepare bytes (add UTF-8 BOM so Excel opens it nicely)
    csv_text = si.getvalue()
    mem = io.BytesIO()
    mem.write(csv_text.encode('utf-8-sig'))
    mem.seek(0)

    filename = f"mh_export_{current_user.username}_{date.today().isoformat()}.csv"
